...
// los métodos antes del main, ya que como, a diferencia de Java, no se pueden declarar statics (que los cargue antes
// del main) hay que hacerlo manualmente (estúpido y sensual Java...)

// como no se le pueden pasar argumentos a los metodos de los timers, definimos las variables que almacenaran la
// posición del mapa en la que se producirá la explosión de manera global, y un par de variables más
s16 posExploX = 0;
s16 posExploY = 0;
...
void teselaBomba(s16 x, s16 y) {             // cambia la tesela en la que está el sprite a la de la bomba
    NF_SetTileOfMap(1, 2, x / 8, y / 8, 2);  // la dos será la tesela de la bomba porque el compilador así lo quiere, y
                                             // nosotros, sus sumisos siervos, obedecemos
...
void explosionRojo() {  // a este método lo llama el TIMER0
    ...
    s16 x = posExploX;
    s16 y = posExploY;
    bool stop = false;  // variable para parar si encuentra un bloque negro, ya que la explosión se acercaria
                        // peligrosamente a los boundaries
    ...
    for (i = 0; i < 3 && !stop; i++) {  // pos X izquierda chequea si no es gris oscuro o negro y lo cambia a rojo
        if (NF_GetTile(0, x + i, y) != 3 && NF_GetTile(0, x + i, y) != 5) {
            ...
            stop = true;  // si encuentra un bloque negro, ya que la explosión se acerca peligrosamente a los
                          // boundaries,para todo con la variable booleana stop
    }
...
void explosionVerde() {  // a este método lo llama el TIMER1, y es el encargado de que vuelva a crecer la hierba donde
                         // antes hubo ceniza
    int i, j;
    s16 x = posExploX;
    s16 y = posExploY;

    // este método cambiará las teselas pertinentes a verde. Como la comprobación de que todo fuera correcto la hemos
    // hecho en el método anterior, el de la explosión a rojo, aqui simplemente nos limitaremos a cambiar aquella tesela
    // que estuviera en rojo a verde
    /*
...
int main(int argc, char **argv) {
    ...
    irqSet(
        IRQ_TIMER1,
        &explosionVerde);  // temporizará el tiempo que están las teselas de la explosión, ya que después se pone verde
    ...
    NF_LoadColisionMap("maps/cmap", 0, 256,
                       264);  // ese 0 es el id del mapa, que será luego usado en el metodo getTite()
    ...
    /**/  // Crea el Sprite del muñequito y el enemigo en la pantalla inferior CreateSprite(pantalla, id_sprite, gfx,
        // id_paleta, x, y);
    ...
    while (!youDie && !enemyDies) {
        ...
        if (keys & KEY_UP) {
            y--;
            if (NF_GetTile(0, x, y) == 3 || NF_GetTile(0, x, y) == 2)
                y++;  // si la tesela es gris claro u oscuro, marcha atrás, primo
        }
        ...
        if (down & KEY_TOUCH && anotherBomb) {
            ...
            posExploX =
                x;  // posExploX/Y son variables globales, para que puedan acceder a ella los métodos de los TIMERS
            posExploY = y;
            anotherBomb = false;  // no volverá a ser true hasta que acabe el proceso de la explosión de la bomba actual
            teselaBomba(posExploX, posExploY);  // llama al método que pone la bomba, que a su vez llama a los demás en
                                                // una vorágine de           llamamiento a métodos hacinados
            TIMER_DATA(0) = 0;
            TIMER_CR(0) =
                TIMER_DIV_1024 | TIMER_ENABLE | TIMER_IRQ_REQ;  // un segundo y llama al método que lo pondrá rojo
        }
        ...
        if (NF_GetTile(0, enemy_x, enemy_y) == 1 && var1 && var2) {  // si es verde, sigue
            enemy_x--;
            if (NF_GetTile(0, enemy_x, enemy_y) == 4)
                enemyDies = true;                        // si es fuego, muere
            if (NF_GetTile(0, enemy_x, enemy_y) != 1) {  // si no es verde cambia de dirección
                var1 = false;  // con las cuatro posibles combinaciones de estas dos variables se computan todos los
        ...
        if (y < 20)
            y = 20;
        if (y > 236 && (x < 112 || x > 128))
            y = 236;  // la puertecita de abajo
        ...
        if (bg_y < 0)
            bg_y = 0;
        if (bg_y > 64)
            bg_y = 64;  // lo justo para que no se vea el otro fondo repetido
        ...
        for (m = -4; m <= 4; m++) {  // para aumentar el radio de alcance del muñequito
            if (spr_x == enemy_x + m && spr_y == enemy_y + m)
                youDie = true;
        }
